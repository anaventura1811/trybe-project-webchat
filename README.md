...
Este projeto foi desenvolvido como requisito parcial para conclusão do módulo de Back-end do curso de Desenvolvimento Web da Trybe. Esta é uma aplicação `Node.js` de _chat_, usando `socket.io` para emitir eventos e atualizar estado no servidor e cliente e o **MongoDB** como banco de dados. Através do cliente é possível enviar e receber mensagens, trocar o nome de usuário, visualizar os usuários online. O MVC é usado para renderizar as mensagens do histórico e usuários online, com ambos vindo direto do servidor.

Neste projeto, desenvolvi um _chat_ online utilizando o pacote `socket.io` do **NodeJS** e, ao utilizar essa aplicação, um usuário é capaz de:
...
- Utilização do pacote `socket.io` do Node.js para criar aplicações que trafeguem mensagens através de sockets.
